...
//		Query q1=em.createQuery("Select count(i) from gst_invoice i,gst_invoice_line il where i.id=il.invoice and i.status='paid' and i.invoice_date between :fd and :td group by il.product  order by count(i)");
//		q1.setParameter("fd", fromDate);
//		q1.setParameter("td", toDate);    /*--*/
//		System.out.println(q1.getResultList());
...
//		Query q2=em.createQuery("Select il.product.name from gst_invoice i,gst_InvoiceLine il where i.id=il.invoice and  i.status='paid' and i.invoice_date between :fd and :td group by il.product.name order by count(i)");
//		q2.setParameter("fd", fromDate);
//		q2.setParameter("td", toDate);          /*--*/
//		System.out.println(q2.getResultList());
...
//		Query q3=em.createQuery("Select i.item.product.category.name from gst_invoice i where i.status='paid' and i.invoice_date between :fd and :td group by i.item.product,i.item.product.category order by count(i)");
//		q3.setParameter("fd", fromDate);      /*--*/
//		q3.setParameter("td", toDate);
